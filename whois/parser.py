...
class WhoisEntry(dict):
    ...
    def parse(self):
        ...
        for attr, regex in list(self._regex.items()):
            if regex:
                ...
                if len(values) == 1 or attr in 'updated_date':
                    values = values[0]
                elif not values:
                    values = None
    ...
    def load(domain, text):
        ...
        """
        # handling for .vn domain
        if(domain.endswith('.vn')):
            res_dict = json.loads(text)
            res_dict.pop('code', None)
            res_dict.pop('totalQuery', None)
            res_dict.pop('creationDate_L', None)
            res_dict.pop('expirationDate_L', None)
            res_dict.pop('rawtext', None)
            res_dict.pop('message', None)
            # rename key
            res_dict["name_servers"] = res_dict.pop("nameServer")
            res_dict["domain_name"] = res_dict.pop("domainName")
            return res_dict
        
        # when use www.whois.com
        if(text.startswith('onclick=')):
            res_dict = {}
            key_delim = '</div><div class="df-row"><div class="df-label">'
            key_delim_len = 48
            val_delim = '</div><div class="df-value">'
            val_delim_len = 28
            l = 0
            while(True):
                if(key_delim not in text[l:]):
                    break
                l = text.index(key_delim, l) + key_delim_len
                val_idx_start = text.index(val_delim, l) + val_delim_len

                key_ = text[l:text.index('</', l)-1]
                val_ = text[val_idx_start: text.index('</', val_idx_start)]

                if("Name Servers" in key_ or "Status" in key_):
                    val_ = val_.split('|')
                res_dict[key_.lower()] = val_

            # rename key
            res_dict["domain_name"] = res_dict.pop("domain")
            res_dict["creation_date"] = res_dict.pop("registered on")
            res_dict["expiration_date"] = res_dict.pop("expires on")
            res_dict["updated_date"] = res_dict.pop("updated on")
            res_dict["name_servers"] = res_dict.pop('name servers')
            res_dict["org"] = res_dict.pop("organization")
            return res_dict
